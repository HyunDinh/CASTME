          ...
          {metrics.map((metric, idx) => (
            ...
            >
              ...
              <div className="flex items-end justify-between">
                ...
                <span
                  className={`text-xs font-bold px-2 py-1 rounded-lg ${metric.isUp ? "bg-emerald-50 text-emerald-600" : "bg-red-50 text-red-600"}`}
                >
                  {metric.trend}
                </span>
              </div>
              ...
              <div className="space-y-4">
                {inProgressJobs.map((campaign) => (
                  <JobCard
                    ...
                    role="shop"
                    onAction={(job) =>
                      router.push(`/my-casting?jobId=${job.id}`)
                    }
                  />
                ))}
              </div>
            ...
            <div className="bg-white/10 backdrop-blur-md rounded-[22px] p-5 h-full">
              <div className="mb-4">
                ...
                <h3 className="text-lg font-extrabold text-white leading-tight">
                  KOC phù hợp nhất với shop của bạn
                </h3>
              </div>
              ...
              <div className="space-y-3">
                {recommendedKocs.map((koc) => (
                  <div
                    key={koc.id}
                    className="bg-white rounded-xl p-3 shadow-sm flex flex-col gap-3"
                  >
                    ...
                    <div className="grid grid-cols-2 gap-2 mt-1">
                      <button className="py-1.5 text-xs font-bold text-gray-600 bg-gray-100 hover:bg-gray-200 rounded-lg transition cursor-pointer">
                        Xem Profile
                      </button>
                      <button className="py-1.5 text-xs font-bold text-white bg-indigo-600 hover:bg-indigo-700 shadow-sm shadow-indigo-200 rounded-lg transition cursor-pointer">
                        Mời hợp tác
                      </button>
                    </div>
                  </div>
