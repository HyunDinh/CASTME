  ...
  if (!creator) {
    return (
      <div className="w-full max-w-5xl mx-auto h-screen flex flex-col items-center justify-center">
        ...
        <button
          onClick={() => router.back()}
          className="px-6 py-2 bg-blue-600 text-white font-bold rounded-xl shadow-lg"
        >
          Quay lại
        </button>
      </div>
          ...
          <div className="flex-1 mb-2 relative z-10 translate-y-[3px]">
            ...
            <div className="flex flex-wrap items-center gap-4 text-sm text-gray-600 font-medium">
              ...
              <span className="flex items-center gap-1 bg-yellow-50 text-yellow-700 px-2.5 py-1 rounded-lg border border-yellow-200">
                ⭐ {creator.stats.averageRating} ({creator.stats.reviewCount}{" "}
                đánh giá)
              </span>
            </div>
          </div>
            ...
            <div className="flex flex-col gap-3">
              {creator.socials.map((s, i) => (
                <a
                  key={i}
                  href={s.link}
                  className="flex items-center justify-between p-3 rounded-xl hover:bg-gray-50 border border-transparent hover:border-gray-100 transition group"
                >
          ...
          <div className="bg-white rounded-3xl p-6 border border-gray-100 shadow-sm">
            ...
            <div className="flex flex-wrap gap-2">
              {creator.styles.map((style, idx) => (
                <span
                  key={idx}
                  className="px-3 py-1.5 bg-blue-50 text-blue-700 font-bold text-xs rounded-lg border border-blue-100"
                >
                  {style}
                </span>
              ))}
              ...
              {activeTab === "portfolio" && (
                <div className="grid grid-cols-2 md:grid-cols-2 gap-4">
                  {creator.gallery.map((img, idx) => (
                    <div
                      key={idx}
                      className="group relative rounded-2xl overflow-hidden aspect-square border border-gray-100 cursor-pointer"
                    >
                      <img
                        src={img}
                        alt={`Gallery ${idx}`}
